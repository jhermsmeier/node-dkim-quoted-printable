...
function decode( value ) {
  ...
  while( ( index = value.indexOf( '=', position ) ) != -1 ) {
    ...
    if( !isEncodedChar( value, index ) )
      throw new Error( 'Unexpected quoted-printable escape' )

    while( isEncodedChar( value, index + offset ) ) {
      offset += 3
    }

    let component = value.slice( index, index + offset )
      .replaceAll( '=', '%' )
